...
data <- read_delim("mcfaul.txt", delim = "*", col_types = col_types)
...
# view(sums)
sums <- sums[-1,] # The German drop words in the tm package are kinda B-, so I lopped off
head <- sums[1:75,] # the first row here, which often had "dass" ("that") at a count of 700+.
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(parubij)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'parubij' (parubiy), RT Search Term: 'mcfaul'",
    x = "Date",
    y = "Frequency"
  ...
  ggsave("mcfaul/parubij.png", width = 7)
```

```{r}
wordcloud(words = head$word, freq = head$count, min.freq = 100,
